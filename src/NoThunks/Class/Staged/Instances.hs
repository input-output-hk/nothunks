...
module NoThunks.Class.Staged.Instances where
...
instance ( NoThunks a
  ...
  wNoThunks =
    let c = constraints @((,) a b)
    in  $$(sgwNoThunks' (allC [|| c ||]))

instance ( NoThunks a
         , NoThunks b
         , NoThunks c
         ) => NoThunks (a, b, c) where
  wNoThunks =
    let c = constraints @((,,) a b c)
    in  $$(sgwNoThunks' (allC [|| c ||]))
...
instance ( NoThunks a
         ...
         , NoThunks c
         , NoThunks d
         ) => NoThunks (a, b, c, d) where
  wNoThunks =
    let c = constraints @((,,,) a b c d)
    in  $$(sgwNoThunks' (allC [|| c ||]))
...
instance ( NoThunks a
         ...
         , NoThunks d
         , NoThunks e
         ) => NoThunks (a, b, c, d, e) where
  wNoThunks =
    let c = constraints @((,,,,) a b c d e)
    in  $$(sgwNoThunks' (allC [|| c ||]))
...
instance ( NoThunks a
         ...
         , NoThunks e
         , NoThunks f
         ) => NoThunks (a, b, c, d, e, f) where
  wNoThunks =
    let c = constraints @((,,,,,) a b c d e f)
    in  $$(sgwNoThunks' (allC [|| c ||]))
...
instance ( NoThunks a
         ...
         , NoThunks f
         , NoThunks g
         ) => NoThunks (a, b, c, d, e, f, g) where
  wNoThunks =
    let c = constraints @((,,,,,,) a b c d e f g)
    in  $$(sgwNoThunks' (allC [|| c ||]))
...
instance NoThunks Void where
  wNoThunks =
    let c = constraints @Void
    in  $$(sgwNoThunks' (allC [|| c ||]))

instance NoThunks () where
  wNoThunks =
    let c = constraints @()
    in  $$(sgwNoThunks' (allC [|| c ||]))

instance NoThunks a => NoThunks [a] where
  wNoThunks =
    let c = constraints @[a]
    in  $$(sgwNoThunks' (allC [|| c ||]))

instance NoThunks a => NoThunks (Maybe a) where
  wNoThunks =
    let c = constraints @(Maybe a)
    in  $$(sgwNoThunks' (allC [|| c ||]))

instance NoThunks a => NoThunks (NonEmpty a) where
  wNoThunks =
    let c = constraints @(NonEmpty a)
    in  $$(sgwNoThunks' (allC [|| c ||]))
