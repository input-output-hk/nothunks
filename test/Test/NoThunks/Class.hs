...
tests = testGroup "NoThunks.Class" [
    ...
    , testGroup "Generics" [
          testProperty "A" $ testWithModel agreeOnContext $ Proxy @UserDefA
        , testProperty "B" $ testWithModel agreeOnContext $ Proxy @UserDefB
        , testProperty "C" $ testWithModel agreeOnContext $ Proxy @UserDefC
        ]
...
{-------------------------------------------------------------------------------
  Check generic instance
-------------------------------------------------------------------------------}

-- | Data-type with a single field
--
-- This is an important case, because the generic representation of
-- @UserDefA x@ is @M1 (M1 (K1 x))@, and if we were to force that to WHNF
-- /before/ checking for thunks, we would end up forcing the @Int@.
data UserDefA = UserDefA Int
  deriving (Generic)

-- | Data-type with two fields
data UserDefB = UserDefB Int Int
  deriving (Generic)

-- | Data-type with two constructors
data UserDefC = UserDefC_Left Int | UserDefC_Right Int
  deriving (Generic)

-- If we leave out the definition of wNoThunks here, thereby relying on the
-- generic instance, the custom error message generated by 'GCheckEdgeCase'
-- kicks in.
instance NoThunks UserDefA where
  wNoThunks ctxt (UserDefA x) = noThunks ctxt x

instance NoThunks UserDefB
instance NoThunks UserDefC

instance FromModel UserDefA where
  data Model UserDefA =
      UserDefA_Thunk (Model UserDefA)
    | UserDefA_Value (Model Int)
    deriving (Show)

  modelIsNF ctxt = \case
      UserDefA_Thunk _ -> NotWHNF ctxt'
      UserDefA_Value x -> constrNF [modelIsNF ctxt' x]
    where
      ctxt' = "UserDefA" : ctxt

  fromModel (UserDefA_Thunk x) k = fromModel x $ \x' ->
                                   k (if ack 3 3 > 0 then x' else x')
  fromModel (UserDefA_Value x) k = fromModel x $ \x' ->
                                   k (UserDefA x')

  genModel = Gen.choice [
        UserDefA_Value <$> genModel
      , UserDefA_Thunk <$> genModel
      ]

instance FromModel UserDefB where
  data Model UserDefB =
      UserDefB_Thunk (Model UserDefB)
    | UserDefB_Value (Model Int) (Model Int)
    deriving (Show)

  modelIsNF ctxt = \case
      UserDefB_Thunk _   -> NotWHNF ctxt'
      UserDefB_Value x y -> constrNF [modelIsNF ctxt' x, modelIsNF ctxt' y]
    where
      ctxt' = "UserDefB" : ctxt

  fromModel (UserDefB_Thunk x)   k = fromModel x $ \x' ->
                                     k (if ack 3 3 > 0 then x' else x')
  fromModel (UserDefB_Value x y) k = fromModel x $ \x' ->
                                     fromModel y $ \y' ->
                                     k (UserDefB x' y')

  genModel = Gen.choice [
        UserDefB_Value <$> genModel <*> genModel
      , UserDefB_Thunk <$> genModel
      ]

instance FromModel UserDefC where
  data Model UserDefC =
      UserDefC_Thunk (Model UserDefC)
    | UserDefC_Value_Left (Model Int)
    | UserDefC_Value_Right (Model Int)
    deriving (Show)

  modelIsNF ctxt = \case
      UserDefC_Thunk _       -> NotWHNF ctxt'
      UserDefC_Value_Left  x -> constrNF [modelIsNF ctxt' x]
      UserDefC_Value_Right x -> constrNF [modelIsNF ctxt' x]
    where
      ctxt' = "UserDefC" : ctxt

  fromModel (UserDefC_Thunk x)       k = fromModel x $ \x' ->
                                         k (if ack 3 3 > 0 then x' else x')
  fromModel (UserDefC_Value_Left  x) k = fromModel x $ \x' ->
                                         k (UserDefC_Left x')
  fromModel (UserDefC_Value_Right x) k = fromModel x $ \x' ->
                                         k (UserDefC_Right x')

  genModel = Gen.choice [
        UserDefC_Value_Left  <$> genModel
      , UserDefC_Value_Right <$> genModel
      , UserDefC_Thunk       <$> genModel
      ]
